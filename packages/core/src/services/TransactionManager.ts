...
/**
 * Manages database transactions for notebook sessions
 * Tracks transaction state, savepoints, and auto-rollback behavior.
 * Delegates SQL syntax generation to the registered TransactionSyntax provider.
 */
export class TransactionManager {
  ...
  private static readonly defaultSyntax: TransactionSyntax = new DefaultTransactionSyntax();

  public static readonly DEFAULT_SAVEPOINT_PREFIX = 'sp_pgstudio_';

  /**
   * Returns the TransactionSyntax for the given engine, falling back to DefaultTransactionSyntax.
   */
  private getTransactionSyntax(engine?: string): TransactionSyntax {
    if (engine) {
      const registry = DriverRegistry.getInstance();
      if (registry.isRegistered(engine)) {
        const syntax = (registry as any).getTransactionSyntax?.(engine);
        if (syntax) {
          return syntax;
        }
      }
    }
    return TransactionManager.defaultSyntax;
  }

  /**
   * Sets the engine for a session (used for TransactionSyntax resolution).
   */
  public setSessionEngine(sessionId: string, engine: string): void {
   ...
   * Delegates to the registered TransactionSyntax if savepoint support is available.
   */
  public async createSavepoint(client: Client | PoolClient, sessionId: string, customName?: string): Promise<string> {
    ...
    const syntax = this.getTransactionSyntax(engine);

    try {
      if (syntax.savepoint) {
        await client.query(syntax.savepoint(savepointName));
      } else {
        // Fallback to standard SAVEPOINT syntax
   ...
   * Delegates to the registered TransactionSyntax if available.
   */
  public async rollbackToSavepoint(client: Client | PoolClient, sessionId: string, savepointName?: string): Promise<void> {
    ...
    try {
      if (syntax.rollbackToSavepoint) {
        await client.query(syntax.rollbackToSavepoint(targetSavepoint));
      } else {
  ...
  public async releaseSavepoint(client: Client | PoolClient, sessionId: string, savepointName?: string): Promise<void> {
    ...
    try {
      if (syntax.releaseSavepoint) {
        await client.query(syntax.releaseSavepoint(targetSavepoint));
      } else {
