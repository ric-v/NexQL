...
import type { PlanMetrics as ProviderPlanMetrics } from '../core/db/ExplainPlanParser';
...
export class QueryAnalyzer {
   ...
   * Delegates to the registered ExplainPlanParser for the given engine if available.
   * Falls back to built-in PostgreSQL JSON EXPLAIN parsing.
   */
  public extractPlanMetrics(explainPlan: any, engine?: string): PlanMetrics | null {
    if (!explainPlan || typeof explainPlan !== 'object') {
      return null;
    }

    // Delegate to registered ExplainPlanParser if available
    if (engine) {
      const registry = DriverRegistry.getInstance();
      if (registry.isRegistered(engine)) {
        const parser = registry.getExplainPlanParser(engine);
        if (parser) {
          const providerMetrics = parser.parsePlan(explainPlan);
          if (providerMetrics) {
            return {
              totalCost: providerMetrics.totalCost,
              planningTime: providerMetrics.planningTime,
              executionTime: providerMetrics.executionTime,
              sequentialScans: providerMetrics.sequentialScans,
              indexScans: providerMetrics.indexScans,
              bufferStats: providerMetrics.bufferStats,
              bottlenecks: providerMetrics.bottlenecks,
              recommendations: providerMetrics.recommendations,
            };
          }
        }
      }
    ...
    // Fallback: built-in PostgreSQL JSON EXPLAIN parsing
