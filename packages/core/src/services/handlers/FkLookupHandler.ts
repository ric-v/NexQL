 ...
 * Delegates SQL generation to SqlTemplateProvider when available.
 */
...
export class FkLookupHandler implements IMessageHandler {
  async handle(message: FkLookupRequest, context: {
    ...
    try {
      ...
      // Resolve engine for potential SqlTemplateProvider delegation
      const engine = resolveDbEngine((connectionConfig as any).engine || metadata.engine || DEFAULT_DB_ENGINE);
      const registry = DriverRegistry.getInstance();

      // Note: FK lookup queries are simple SELECT queries that work across engines.
      // The SqlTemplateProvider doesn't currently define FK lookup methods,
      // but we resolve the engine for future extensibility.
      let query: string;
      let params: any[];
