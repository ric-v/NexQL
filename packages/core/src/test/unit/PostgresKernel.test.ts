...
describe('PostgresKernel', () => {
  ...
  beforeEach(() => {
    ...
    configGetStub = sandbox.stub().callsFake((key: string, defaultValue?: unknown) => {
      if (key === 'nexql.connections') {
        return connectionList;
      }
      if (key === 'nexql.query.autoLimitEnabled') {
        return false;
      }
      if (key === 'nexql.performance.defaultLimit') {
        return typeof defaultValue === 'number' ? defaultValue : 1000;
      }
      if (key === 'nexql.performance.slowQueryThresholdMs') {
  ...
  it('should execute query successfully', async () => {
    ...
    const notebookOut = allCalls.map((c: any) => c.args[0]).find((out: any) =>
      out.items[0].mime === 'application/vnd.nexql-notebook.result'
    );
  ...
  it('should handle serialization errors in query results', async () => {
    ...
    const allCalls = execution.appendOutput.getCalls();
    const notebookOut = allCalls.map((c: any) => c.args[0]).find((out: any) =>
      out.items[0].mime === 'application/vnd.nexql-notebook.error'
    );
    expect(notebookOut).to.exist;
    expect(notebookOut.items[0].mime).to.equal('application/vnd.nexql-notebook.error');
