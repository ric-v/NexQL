...
describe('NotebookStatusBar', () => {
  ...
  it('hides items when no notebook is active and shows the no-connection state', () => {
    ...
    (vscode.window as any).activeNotebookEditor = createNotebookEditor('nexql-query', {});
    statusBar.update();
  ...
  it('shows connection, profile, risk, and transaction indicators for an active notebook', () => {
    ...
    (vscode.window as any).activeNotebookEditor = createNotebookEditor('nexql-notebook', {
      connectionId: 'c1',
      databaseName: 'appdb'
    }, 'nb:active');
