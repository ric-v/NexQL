...
export class PostgresDriver implements DbDriver {
  ...
  /**
   * Pool cache key. Includes a fingerprint of the resolved password and
   * other auth-affecting fields so that rotating credentials (or changing
   * SSL mode / SSH tunnel settings) invalidates the cache instead of
   * silently reusing a pool created with stale credentials.
   */
  private getConnectionKey(config: ConnectionConfig): string {
    const password = coerceConnectionPassword((config as any).password);
    const authFingerprint = password
    ...
    const sshFingerprint = config.ssh?.enabled
      ? createHash('sha256')
          .update(`${config.ssh.host}:${config.ssh.port}:${config.ssh.username}:${config.ssh.privateKeyPath ?? ''}`)
          .digest('hex')
          .slice(0, 16)
      : 'nossh';
    return `${config.id}:${config.database || 'postgres'}:${sslmode}:${sshFingerprint}:${authFingerprint}`;
  }

  private async createClientConfig(config: ConnectionConfig, forceDisableSSL: boolean = false): Promise<ClientConfig> {
    // Password priority: config.password (already enriched by ConnectionManager) > SecretStorage > .pgpass
    // ConnectionManager.enrichConfigWithPassword() resolves the password before calling the driver,
    // so config.password should already be set in most cases.
    let password: string | undefined = coerceConnectionPassword((config as any).password);

    if (!password && config.username && config.id) {
      try {
        password = await SecretStorageService.getInstance().getPassword(config.id);
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[PostgresDriver] Failed to retrieve password from SecretStorage for connection ${config.id}: ${message}`);
      }
    }
    ...
    if (!password && config.username) {
      ...
      if (pgpassPwd !== undefined) {
        password = pgpassPwd;
        console.log(`[PostgresDriver] Password resolved from .pgpass for ${config.username}@${config.host}:${config.port}/${targetDb}`);
      } else if (targetDb !== 'postgres') {
        const fallback = resolvePgPassPassword(config.host, config.port, 'postgres', config.username);
        if (fallback !== undefined) {
          password = fallback;
          console.log(`[PostgresDriver] Password resolved from .pgpass (postgres fallback) for ${config.username}@${config.host}:${config.port}`);
        }
      }

      if (!password) {
        console.log(`[PostgresDriver] No password found on config, SecretStorage, or .pgpass for ${config.username}@${config.host}. Expected: ${pgPassFileDescription()}`);
      }
    }
    ...
    const clientConfig: ClientConfig = {
      ...
      statement_timeout: config.statementTimeout || vscode.workspace.getConfiguration('nexql').get<number>('queryTimeout') || undefined,
