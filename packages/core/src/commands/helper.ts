...
/** Metadata persisted into `.pgsql` scratch files (must include `engine` for SqlExecutor / kernel routing). */
function scratchNotebookFileMetadata(
  ...
  return {
    ...
    engine: resolveDbEngine((metadata?.engine as string | undefined) || DEFAULT_DB_ENGINE),
    host: metadata?.host,
    port: metadata?.port,
...
export class NotebookBuilder {
  ...
  async showNew(): Promise<void> {
    if (_extensionContext && this.databaseName) {
      ...
      const fileMetadata = scratchNotebookFileMetadata(
        this.metadata,
        this.databaseName!,
        this._scratchTitle(),
      );
