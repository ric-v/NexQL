...
export class AiService {
  ...
  buildSystemPrompt(selectedDbEngine?: DbEngine | 'generic'): string {
    ...
    if (effectiveEngine !== 'generic') {
      ...
      if (registry.isRegistered(effectiveEngine)) {
        const dialect = registry.getDialect(effectiveEngine);
        const addendum = dialect.buildSystemPromptAddendum?.();
        if (addendum) {
...
- If syntax differs by engine, explicitly call out differences for the supported engines.
- Do not assume any engine-specific features unless the user requests them.
...
The UI will automatically parse this and show clickable suggestion bubbles.`;
  ...
  async callDirectApi(provider: string, userMessage: string, config: vscode.WorkspaceConfiguration, customSystemPrompt?: string): Promise<{ text: string, usage?: string }> {
    ...
    if (!apiKey && provider !== 'custom' && provider !== 'ollama' && provider !== 'lmstudio' && provider !== 'github') {
      throw new Error(`API Key is required for ${provider} provider. Please configure nexql.aiApiKey.`);
    }
