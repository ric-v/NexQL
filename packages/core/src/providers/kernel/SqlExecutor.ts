...
function detachPgNoticeListener(client: { removeListener?: Function; off?: Function }, listener: (...args: any[]) => void): void {
  if (typeof client.removeListener === 'function') {
    client.removeListener('notice', listener);
...
export class SqlExecutor {
  ...
  private static readonly REVIEW_SHOWN_KEY = 'nexql.reviewPrompt.shown';
  ...
  private async promptForNamedParameterValues(paramNames: string[]): Promise<unknown[] | undefined> {
    const paramsConfig = vscode.workspace.getConfiguration('nexql.parameters');
  ...
  private applyAutoLimit(query: string, connection: any, notebookMetadata?: any, profileContext?: any): string {
    ...
    if (profileContext?.autoLimitSelectResults !== undefined && profileContext.autoLimitSelectResults > 0) {
      ...
      const autoLimitEnabled = vscode.workspace.getConfiguration()
        .get<boolean>('nexql.query.autoLimitEnabled', true);

      if (autoLimitEnabled || connection.readOnlyMode) {
        limit = vscode.workspace.getConfiguration()
          .get<number>('nexql.performance.defaultLimit', 1000);
      }
    }
    ...
    try {
      ...
      for (let stmtIndex = 0; stmtIndex < statements.length; stmtIndex++) {
        ...
        try {
          ...
          const success = true;
          const slowThresholdMs = vscode.workspace.getConfiguration().get<number>('nexql.performance.slowQueryThresholdMs', 2000);
