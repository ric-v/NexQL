...
import type { CompletionProvider as DbCompletionProvider } from '../core/db/CompletionProvider';
...
export class SqlCompletionProvider implements vscode.CompletionItemProvider {
  ...
  async provideCompletionItems(
    ...
    try {
      ...
      // Add SQL keywords (engine-specific if available)
      completionItems.push(...this._getSqlKeywords(engine));
  ...
  private static readonly defaultCompletionProvider: DbCompletionProvider = new DefaultCompletionProvider();

  /**
   * Returns the CompletionProvider for the given engine, falling back to DefaultCompletionProvider.
   */
  private _getDbCompletionProvider(engine: string): DbCompletionProvider {
    const registry = DriverRegistry.getInstance();
    if (registry.isRegistered(engine)) {
      const provider = registry.getCompletionProvider(engine);
      if (provider) {
        return provider;
      }
    }
    return SqlCompletionProvider.defaultCompletionProvider;
  }

  private _getSqlKeywords(engine?: string): vscode.CompletionItem[] {
    const completionProvider = this._getDbCompletionProvider(engine || DEFAULT_DB_ENGINE);
    const keywords = completionProvider.getKeywords();
    const builtinFunctions = completionProvider.getBuiltinFunctions();
