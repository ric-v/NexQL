...
import type { EngineChangeEvent } from '../core/api/ProviderAPI';
...
export class DatabaseTreeProvider implements vscode.TreeDataProvider<DatabaseTreeItem> {
  ...
  private static readonly FAVORITES_KEY = 'nexql.favorites';
  private static readonly RECENT_KEY = 'nexql.recentItems';
  ...
  private readonly _registrySubscription: vscode.Disposable;
  ...
  constructor(private readonly extensionContext: vscode.ExtensionContext) {
    ...
    // Listen to DriverRegistry engine changes to refresh tree
    this._registrySubscription = DriverRegistry.getInstance().onDidChangeEngines(
      (event: EngineChangeEvent) => {
        // Refresh the entire tree when engines are added or removed
  ...
  dispose(): void {
    this._registrySubscription.dispose();
  ...
  public async getDbObjectsForConnection(connection: any): Promise<Array<{ type: string, schema: string, name: string, columns?: string[] }>> {
    ...
    const introspection = registry.getIntrospection(engine);
    ...
    try {
      ...
      const tablesQuery = engine === 'postgres'
        ? `
        SELECT 
          t.table_schema,
          t.table_name,
        ...
        LIMIT 100
      `
        : introspection.listTables?.(connection.database || 'information_schema') || '';
      ...
      const viewsQuery = engine === 'postgres'
        ? `
        SELECT 
          t.table_schema,
          t.table_name,
  ...
  async getChildren(element?: DatabaseTreeItem): Promise<DatabaseTreeItem[]> {
    ...
    if (!element) {
      ...
      ungroupedConnections.forEach(conn => {
        const connEngine = conn.engine || 'postgres';
        const connRegistry = DriverRegistry.getInstance();
        let engineDisplayName = connEngine;
        try {
          const reg = connRegistry.isRegistered(connEngine);
          if (reg) {
            // Use the engine identifier as display name fallback
            engineDisplayName = connEngine.charAt(0).toUpperCase() + connEngine.slice(1);
          }
        } catch { /* use default */ }
        const connLabel = `${conn.name || `${conn.host}:${conn.port}`} [${engineDisplayName}]`;
        rootItems.push(new DatabaseTreeItem(
          connLabel,
          vscode.TreeItemCollapsibleState.Collapsed,
          'connection',
    ...
    if (element.type === 'connection-group') {
      ...
      return groupConnections.map(conn => {
        const connEngine = conn.engine || 'postgres';
        let engineDisplayName = connEngine.charAt(0).toUpperCase() + connEngine.slice(1);
        const connLabel = `${conn.name || `${conn.host}:${conn.port}`} [${engineDisplayName}]`;
    ...
    const engine = connection.engine || 'postgres';
    const introspection = registry.getIntrospection(engine);
    const featureFlags = registry.getFeatureFlags(engine);
      ...
      switch (element.type) {
        case 'connection':
          ...
          if (engine === 'postgres') {
            ...
            if (featureFlags.supportsTablespaces) {
            ...
            const databasesResult = await client.query(introspection.listSchemas?.() || 'SELECT 1');
        ...
        case 'databases-group':
          ...
          const dbResult = await this._cache.getOrFetch(cacheKey, async () => {
            ...
            if (introspection.listSchemas) {
              return await client!.query(introspection.listSchemas());
            }

            return await client!.query('SELECT 1 AS datname');
          });
          const systemDatabases = engine === 'postgres'
            ? dbResult.rows.filter((row: any) => SYSTEM_DATABASES.has(row.datname))
            : [];
          const userDatabases = engine === 'postgres'
            ? dbResult.rows.filter((row: any) => !SYSTEM_DATABASES.has(row.datname))
            : dbResult.rows;
        ...
        case 'database':
          ...
          // If engine does not support schemas, display tables directly under database
          if (!featureFlags.supportsSchemas) {
            // No schema level — show tables/views directly under database
            const directItems: DatabaseTreeItem[] = [
              new DatabaseTreeItem('Tables', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined),
              new DatabaseTreeItem('Views', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined),
            ];
            if (featureFlags.supportsStoredProcedures) {
              directItems.push(new DatabaseTreeItem('Functions', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.databaseName));
            }
            return directItems;
          }
          ...
          const databaseCategoryItems: DatabaseTreeItem[] = [
          ...
          // Only show Extensions if the engine supports them
          ...
          databaseCategoryItems.push(new DatabaseTreeItem('Extensions', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, extensionCountResult.rows[0].count));

          // Cron Jobs — only if supportsPgCron
          if (featureFlags.supportsPgCron) {
            let cronJobCount = 0;
            try {
            ...
            databaseCategoryItems.push(new DatabaseTreeItem('Cron Jobs', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, cronJobCount));
          }

          // Foreign Data Wrappers — only if supportsForeignDataWrappers
          if (featureFlags.supportsForeignDataWrappers) {
            const fdwCountResult = await client.query('SELECT COUNT(*) FROM pg_foreign_data_wrapper');
            databaseCategoryItems.push(new DatabaseTreeItem('Foreign Data Wrappers', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, fdwCountResult.rows[0].count));
          }

          // Event Triggers — only if supportsEventTriggers
          if (featureFlags.supportsEventTriggers) {
            const eventTriggerCountResult = await client.query('SELECT COUNT(*) FROM pg_event_trigger');
            databaseCategoryItems.push(new DatabaseTreeItem('Event Triggers', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, eventTriggerCountResult.rows[0].count));
          }

          // Logical Replication (Publications/Subscriptions) — only if supportsLogicalReplication
          if (featureFlags.supportsLogicalReplication) {
            const publicationCountResult = await client.query('SELECT COUNT(*) FROM pg_publication');
            databaseCategoryItems.push(new DatabaseTreeItem('Publications', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, publicationCountResult.rows[0].count));
            ...
            databaseCategoryItems.push(new DatabaseTreeItem('Subscriptions', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, subscriptionCount));
          }

          return databaseCategoryItems;
...
i.relname as index_name,
  ...
  CASE WHEN e.installed_version IS NOT NULL THEN true ELSE false END as is_installed
            ...
            case 'Tables':
              const tableResult = engine === 'postgres'
                ? await client.query(
                    `SELECT 
                       t.table_name,
                ...
                : await client.query(introspection.listTables?.(element.schema) || 'SELECT 1 AS table_name');
            ...
            case 'Views':
              const viewResult = await client.query(
                ...
                engine === 'sqlite' ? undefined : [element.schema]
              );
        ...
        case 'schema':
          if (engine !== 'postgres') {
            const tablesQuery = introspection.listTables?.(element.schema) || 'SELECT 1 AS table_name';
            const tablesResult = await client.query(tablesQuery);
            const viewsQuery = engine === 'sqlite'
              ? "SELECT name AS table_name FROM sqlite_master WHERE type = 'view' ORDER BY name"
              : "SELECT table_name FROM information_schema.views WHERE table_schema = $1 ORDER BY table_name";
            const viewsResult = await client.query(viewsQuery, engine === 'sqlite' ? undefined : [element.schema]);
          ...
          if (featureFlags.supportsStoredProcedures) {
            schemaItems.push(new DatabaseTreeItem('Procedures', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, proceduresCountResult.rows[0].count));
          }
          if (featureFlags.supportsMaterializedViews) {
            schemaItems.push(new DatabaseTreeItem('Materialized Views', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, materializedViewsCountResult.rows[0].count));
          }
          if (featureFlags.supportsCustomTypes) {
            schemaItems.push(new DatabaseTreeItem('Types', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, typesCountResult.rows[0].count));
          }
          if (featureFlags.supportsForeignDataWrappers) {
            schemaItems.push(new DatabaseTreeItem('Foreign Tables', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, foreignTablesCountResult.rows[0].count));
          }
          if (featureFlags.supportsSequences) {
            schemaItems.push(new DatabaseTreeItem('Sequences', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, seqCountResult.rows[0].count));
          }
          if (featureFlags.supportsTriggers) {
            schemaItems.push(new DatabaseTreeItem('Triggers', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, trigCountResult.rows[0].count));
          }
          if (featureFlags.supportsDomains) {
            schemaItems.push(new DatabaseTreeItem('Domains', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, domCountResult.rows[0].count));
          }
          if (featureFlags.supportsAggregates) {
            schemaItems.push(new DatabaseTreeItem('Aggregates', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, aggCountResult.rows[0].count));
          }
          if (featureFlags.supportsRules) {
            schemaItems.push(new DatabaseTreeItem('Rules', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, ruleCountResult.rows[0].count));
          }
        ...
        case 'table': {
          const tableSubItems: DatabaseTreeItem[] = [
          ...
          if (featureFlags.supportsTriggers) {
            tableSubItems.push(new DatabaseTreeItem('Triggers', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, element.label));
          }
          if (featureFlags.supportsRlsPolicies) {
            ...
            tableSubItems.push(new DatabaseTreeItem(
              'RLS Policies',
              vscode.TreeItemCollapsibleState.Collapsed,
              'category',
              element.connectionId,
          ...
          if (featureFlags.supportsRules) {
            tableSubItems.push(new DatabaseTreeItem('Rules', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, element.label));
          }
          if (featureFlags.supportsPartitions) {
            tableSubItems.push(new DatabaseTreeItem('Partitions', vscode.TreeItemCollapsibleState.Collapsed, 'category', element.connectionId, element.databaseName, element.schema, element.label));
          }
          return tableSubItems;
        }
...
export class DatabaseTreeItem extends vscode.TreeItem {
  constructor(
    ...
    public readonly engine?: string,  // Database engine identifier for visual indicators
  ) {
    ...
    this.description = this.getDescription(type, isInstalled, installedVersion, roleAttributes, isFavorite, count, rowCount, size, environment, readOnlyMode, engine);
    this.iconPath = this.resolveIcon(type, isDisconnected, isInstalled, engine);
  }

  /**
   * Resolve the icon for this tree item.
   * For connection nodes, uses engine-specific icons when available.
   */
  private resolveIcon(
    type: string,
    ...
    // Engine-specific icons for connection nodes
    if (type === 'connection' && engine) {
      const engineIconMap: Record<string, { icon: string; color: string }> = {
        postgres: { icon: 'database', color: 'charts.blue' },
        mysql: { icon: 'database', color: 'charts.orange' },
        sqlite: { icon: 'database', color: 'charts.green' },
        mssql: { icon: 'database', color: 'charts.red' },
        oracle: { icon: 'database', color: 'charts.purple' },
      };
      const engineIcon = engineIconMap[engine];
      if (engineIcon) {
        const color = isDisconnected
          ? new vscode.ThemeColor('disabledForeground')
          : new vscode.ThemeColor(engineIcon.color);
        return new vscode.ThemeIcon(engineIcon.icon, color);
      }
      // Fallback for unknown engines
      return new vscode.ThemeIcon('plug', isDisconnected ? new vscode.ThemeColor('disabledForeground') : new vscode.ThemeColor('charts.blue'));
    }

    const iconMap: Record<string, vscode.ThemeIcon | undefined> = {
  ...
  private getDescription(type: string, isInstalled?: boolean, installedVersion?: string, roleAttributes?: { [key: string]: boolean }, isFavorite?: boolean, count?: number, rowCount?: string | number, size?: string, environment?: string, readOnlyMode?: boolean, engine?: string): string | undefined {
    let desc: string | undefined = undefined;

    if (type === 'connection') {
      const badges = [];
      // Show engine name badge
      if (engine) {
        badges.push(engine.charAt(0).toUpperCase() + engine.slice(1));
      }
