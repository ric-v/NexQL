...
export function registerProviders(context: vscode.ExtensionContext, outputChannel: vscode.OutputChannel) {
  ...
  const treeView = vscode.window.createTreeView('nexqlExplorer', {
  ...
  treeView.onDidChangeSelection(e => {
    if (e.selection.length > 0) {
      ...
      vscode.commands.executeCommand('setContext', 'nexql.isFavorite', item.isFavorite === true);
    } else {
      vscode.commands.executeCommand('setContext', 'nexql.isFavorite', false);
    }
  });
  ...
  context.subscriptions.push(
    vscode.workspace.registerNotebookSerializer('nexql-notebook', notebookProvider),
    vscode.workspace.registerNotebookSerializer('nexql-query', new PostgresNotebookSerializer())
  );
  ...
  runDeferredProviderTask(outputChannel, 'registerQueryHistoryProvider', async () => {
    ...
    context.subscriptions.push(
      vscode.window.registerTreeDataProvider('nexql.history', queryHistoryProvider)
    );
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('nexql.savedQueries', savedQueriesTreeProvider)
  );
  ...
  const notebooksTreeView = vscode.window.createTreeView('nexql.notebooks', {
