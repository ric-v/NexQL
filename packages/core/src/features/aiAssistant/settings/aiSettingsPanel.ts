...
export class AiSettingsPanel {
          ...
          case 'saveSettings':
            try {
              ...
              if (settings.provider === 'github') {
                await this._extensionContext.secrets.delete('nexql.aiApiKey');
              } else if (settings.apiKey) {
                await this._extensionContext.secrets.store('nexql.aiApiKey', settings.apiKey);
              } else {
                await this._extensionContext.secrets.delete('nexql.aiApiKey');
              }
  ...
  private async _sendSettingsLoaded(): Promise<void> {
    ...
    const apiKey = await this._extensionContext.secrets.get('nexql.aiApiKey');
